    ...
    for (let x = 0; x < BoardWidth; x++) {
      ...
      switch (Terrain[y][x]) {
        ...
        case '?':
          ...
          //console.log(`grabbed ${tileIndex+1} of ${tilePile.length}, tile remaining:${tilePile}`);
          switch(newTile){
      ...
      if (Terrain[y][x]==='/' ||Terrain[y][x]==='d') {
        ...
        console.log(`grabbed ${numberIndex+1} of ${numberPile.length}, tile remaining:${numberPile}`);
      }
      row.push( 
        <div 
      ...
      if(x === BoardWidth-1){
        row.push(addSettleSpot(xCoord - ssRadius + HexWidth,yCoord - ssRadius + HexHeight*0.25,`s:${x},${y},2`));
        row.push(addSettleSpot(xCoord - ssRadius + HexWidth,yCoord - ssRadius+HexHeight*0.75,`s:${x},${y},3`));
      }

      //add bottom left for the first hexes on each row and the entire last row
      if(y===BoardHeight-1||x===0){ row.push(addSettleSpot(xCoord - ssRadius,yCoord - ssRadius + HexHeight*0.75,`s:${x},${y},4`)); }
      //add bottom centers for the last row of hexes
      if(y === BoardHeight-1){row.push(addSettleSpot(xCoord - ssRadius + HexWidth*0.5,yCoord - ssRadius+HexHeight,`s:${x},${y},5`));}
